...
# 在企业代理/防火墙（自签名证书）环境下，禁用SSL证书验证警告
# 仅当环境变量 CURL_CA_BUNDLE 为空时启用（即Docker中已设置跳过验证）
if os.getenv("CURL_CA_BUNDLE", "DEFAULT") == "" or os.getenv("REQUESTS_CA_BUNDLE", "DEFAULT") == "":
    urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
    os.environ["PYTHONHTTPSVERIFY"] = "0"
    # 为 httpx 客户端创建不验证SSL的上下文
    import httpx
    _ssl_context = ssl.create_default_context()
    _ssl_context.check_hostname = False
    _ssl_context.verify_mode = ssl.CERT_NONE
    # 全局设置 httpx 使用不验证SSL的客户端
...
class LangGraphConfig:
    ...
    def get_llm_config(cls) -> Dict[str, Any]:
        ...
        # 在企业代理/防火墙环境下，为httpx客户端禁用SSL验证
        if os.getenv("CURL_CA_BUNDLE", "DEFAULT") == "" or os.getenv("REQUESTS_CA_BUNDLE", "DEFAULT") == "":
            try:
                import httpx
                config["http_client"] = httpx.Client(verify=False)
                config["http_async_client"] = httpx.AsyncClient(verify=False)
            except Exception:
                pass
        return config
